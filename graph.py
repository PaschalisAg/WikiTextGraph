...
def generate_graph(
    ...
    for batch_index, batch in enumerate(parquet_file.iter_batches(batch_size=50_000)):
        ...
        df = df[~df['title'].apply(lambda s: bool(isinstance(s, str) and filter_re.search(s)))]

        df['Redirect_Flag'] = df['text'].str.lower().str.startswith(tuple(redirect_keywords)).astype(int)
        ...
        graph_data = graph_data[~graph_data['Target'].str.match(lang_link_pattern)]
